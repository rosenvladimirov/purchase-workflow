...
class ProductSetAdd(models.TransientModel):
    ...
    _description = "Wizard model to add product set into a purchase"
    ...
    def _compute_product_set_line_ids(self):
        line_ids = self.env.context.get("purchase_product_set_add__set_line_ids", [])
    ...
    def add_set(self):
        ...
        if order_lines:
            ...
            values.update(self.product_set_id.prepare_purchase_order_values(1))
            product_set_section_id = self.order_id.order_line.create(values)
            for line in order_lines:
                line[2]["product_set_section_id"] = product_set_section_id.id
    ...
    def _prepare_order_lines(self):
        ...
        for seq, set_line in enumerate(self._get_lines(), start=1):
            values = self.prepare_purchase_order_line_data(set_line)
    ...
    def prepare_purchase_order_line_data(self, set_line, max_sequence=0):
        self.ensure_one()
        line_values = set_line.prepare_purchase_order_line_values(
