...
class PurchaseOrder(models.Model):
    ...
    product_set_ids = fields.Many2many('purchase.product.set',
        ...
        for record in self:
            ...
            for product_set_section_id, lines in groupby(order_lines.sorted(lambda r: r.product_set_section_id.id),
                ...
                for order_line_id in lines:
                    if order_line_id.qty_received != 0.0 and order_line_id.order_id.invoice_status == 'invoiced':
                    ...
                    if order_line_id.product_id.id == compensation_product_id:
                        if order_line_id.qty_received != 0.0 and order_line_id.order_id.invoice_status == 'invoiced':
                            total_quantity['section_quantity'] += order_line_id.qty_received
                        else:
            ...
            for product_set_id, total_quantity in product_set_ids.items():
                product_set_ids = self.env['purchase.product.set'].search([
                    ('order_id', '=', record.id),
                ...
                if product_set_ids:
                    ...
                    product_set_ids.with_context(**dict(self._context, create_new_set=True)).write(
                        record.product_set_ids._get_purchase_product_set_value(
                            record,
                            product_set_id,
                    ...
                    record.product_set_ids |= self.env['purchase.product.set']. \
    ...
    def action_set_sections(self):
        ...
        for record in self:
            ...
            self.env['purchase.product.set'].search([('order_id', '=', record.id)]).with_context(**dict(self._context, create_new_set=True)).unlink()
            values = {}
            total_quantity = {}
                ...
                if line.product_set_id != product_set_id:
                    ...
                    if product_set_id:
                        values[f'section_{line.id}'] = product_set_id.\
                            prepare_purchase_order_values(sequence + add_sequence, order_id=record)
                    else:
            ...
            for line in record.order_line:
                if values.get(f'section_{line.id}') and not values.get(line.product_set_id):
                    ...
                    _logger.info(f"Create section: {values[f'section_{line.id}']}")
                    values[line.product_set_id] = {
